...
function build_metis_graph(tree::SimpleWeightedGraph,
                         metis_opts::Int)
    file_name = joinpath(source_dir, "metis_graph" * string(metis_opts) * "." * string(uuid4()) * ".gr")
    f = open(file_name, "w")
    wts = tree.weights
...
function metis(metis_path::String,
            ...
            ub_factor::Int,
            metis_opts::Int)
    log_file = joinpath(source_dir, "metis" * string(metis_opts) * "." * string(uuid4()) * ".log.txt")
    ...
    try; rm(log_file; force=true); catch; end
end