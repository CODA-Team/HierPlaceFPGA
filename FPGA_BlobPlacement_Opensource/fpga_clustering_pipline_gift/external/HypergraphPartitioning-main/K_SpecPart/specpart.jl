...
function two_way_spectral_refine(hypergraph_file::String,
    ...
    for iter in 1:refine_iters
        ...
        refined_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
    ...
    @info "Running optimal attempt partitioning**"
    specpart_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
...
# =========================================================================
# Feature-based variants: use GIFT coordinates instead of solve_eigs()
# =========================================================================

function two_way_spectral_refine_with_features(hypergraph_file::String,
                                partition::Vector{Int},
                                hgraph::__hypergraph__,
                                metis_path::String,
                                ub_factor::Int,
                                ...
                                eigen_vecs::Int = 1,
                                solver_iters::Int = 20,
                                specpart_mode::String = "gift_single")
    line_log = repeat("=", 60)
    @info "$line_log"
    @info "**Using GIFT features (2-way), mode=$specpart_mode**"
    @info "$line_log"
    hgraph = remove_single_hyperedges(hgraph)
    processed_hg_name = hypergraph_file * ".processed"
    pre_refined_part = deepcopy(partition)
    ...
    actual_iters = specpart_mode == "gift_single" ? 1 : refine_iters
    @info "[specpart-features-2way] mode=$specpart_mode, actual_iters=$actual_iters"

    for iter in 1:actual_iters
        @info "[specpart-features-2way] iteration $iter / $actual_iters"
        try

        # Decide embedding: iter 1 always uses GIFT; iter 2+ uses solve_eigs in gift_then_spectral mode
        if iter == 1 || specpart_mode == "gift_single"
            embedding = features  # n×2 GIFT coordinates
            @info "[specpart-features-2way] using GIFT features as embedding"
        else
            # gift_then_spectral: use original spectral method
            @info "[specpart-features-2way] using solve_eigs (spectral) for iteration $iter"
            side_0 = findall(x-> x==0, specpart_refined_partition)
            side_1 = findall(x-> x==1, specpart_refined_partition)
            fixed_vertices_eig = __pindex__(side_0, side_1)
            embedding = solve_eigs(hgraph,
                                adj_matrix,
                                fixed_vertices_eig,
                                false,
                                ...
                                solver_iters)
            # Ensure 2D
            if ndims(embedding) == 1
                embedding = reshape(embedding, :, 1)
            end
        end
        ...
        if isempty(partitions_vec)
            @warn "[specpart-features-2way] iteration $iter: tree_partition returned empty, skipping"
            continue
        end
        ...
        try run(`sh -c "rm -f *.part.2"`, wait=true) catch end
        for i in 1:length(partitions)
            (cutsize, balance) = golden_evaluator(hgraph, num_parts, partitions[i])
            push!(cutsizes, cutsize)
            if haskey(cut_dictionary, cutsize) == false
            ...
            @info "[specpart-features-2way] Refined partition $i with cutsize $cutsize $balance"
        end
        ...
        for i in 1:solns_to_pick
            push!(best_partitions, unique_partitions[sorted_partition_ids[i]])
            @info "[specpart-features-2way] partition picked with cutsize $(unique_cutsizes[sorted_partition_ids[i]])"
        end
        push!(best_partitions, partition)
        hgraph_contracted, clusters = overlay(best_partitions, hgraph)
        @info "Running optimal attempt partitioning**"
        refined_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
        cutsize = golden_evaluator(hgraph_contracted, num_parts, refined_partition)
        @info "specpart-features-2way cutsize recorded: $cutsize"
        partition_projected = zeros(Int, hgraph.num_vertices)
        for i in 1:length(clusters)
            cid = clusters[i]
            partition_projected[i] = refined_partition[cid]
        ...
        push!(global_cutsizes, cutsize[1])
            ...
            @warn "[specpart-features-2way] iteration $iter FAILED: $(sprint(showerror, iter_e))"
            try run(`sh -c "rm -f *.part.2"`, wait=true) catch end
            continue
        end
    end
    # global overlaying
    @info "[specpart-features-2way] running final round of overlay"
    push!(global_partitions, partition)
    hgraph_contracted, clusters = overlay(global_partitions, hgraph)
    @info "Running optimal attempt partitioning**"
    specpart_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
    cutsize = golden_evaluator(hgraph_contracted, num_parts, specpart_partition)
    @info "[specpart-features-2way] refined cutsize recorded: $cutsize"
    partition_projected = zeros(Int, hgraph.num_vertices)
    for i in 1:length(clusters)
        cid = clusters[i]
        partition_projected[i] = specpart_partition[cid]
    ...
    partition_projected = read_hint_file(specpart_partition_name)
    cutsize = golden_evaluator(hgraph, num_parts, partition_projected)
    if isempty(global_cutsizes)
        @info "specpart-features-2way cutsize recorded (no refine iters succeeded): $(cutsize[1])"
    ...
    if pre_refined_cut < post_refined_cut
        @info "specpart-features-2way cutsize recorded: $pre_refined_cut"
        return pre_refined_part, pre_refined_cut
    else
        @info "specpart-features-2way cutsize recorded: $post_refined_cut"
        return post_refined_part, post_refined_cut
    end
end

function k_way_spectral_refine_with_features(hypergraph_file::String,
                            partition::Vector{Int},
                            hgraph::__hypergraph__,
                            metis_path::String,
                            ub_factor::Int,
    ...
    line_log = repeat("=", 60)
    @info "$line_log"
    @info "**Using GIFT features (K-way), mode=$specpart_mode**"
    @info "$line_log"
    specpart_refined_partition = partition
    processed_hg_name = hypergraph_file * ".processed"
    write_hypergraph(hgraph, processed_hg_name)
    ...
    actual_iters = specpart_mode == "gift_single" ? 1 : refine_iters
    @info "[specpart-features-kway] mode=$specpart_mode, actual_iters=$actual_iters"

    for iter in 1:actual_iters
        @info "[specpart-features-kway] iteration $iter / $actual_iters"
        try

        # Decide embedding
        if iter == 1 || specpart_mode == "gift_single"
            # Use GIFT features directly
            reduced_evec_matrix = features  # n×2
            @info "[specpart-features-kway] using GIFT features as embedding"
        else
            # gift_then_spectral: use original spectral method (solve_eigs + LDA)
            @info "[specpart-features-kway] using solve_eigs + LDA (spectral) for iteration $iter"
            partition_list = [Vector{Int}() for i in 1:num_parts]
            for i in 1:length(specpart_refined_partition)
                push!(partition_list[specpart_refined_partition[i]+1], i)
            end
            pindices_vec = []
            for i in 1:num_parts
                side_0 = partition_list[i]
            ...
            @sync Threads.@threads for i in 1:length(pindices_vec)
                emb = solve_eigs(hgraph,
                                ...
                                solver_iters,
                                epsilon=num_parts-1)
                push!(embedding_vec, emb)
                push!(order_vec, i)
            end
            ordered_vec_idxs = sortperm(order_vec)
            concatenated_evec_matrix = embedding_vec[ordered_vec_idxs[1]]
            for i in 2:length(ordered_vec_idxs)
                concatenated_evec_matrix = hcat(concatenated_evec_matrix, embedding_vec[ordered_vec_idxs[i]])
            end
            reduced_evec_matrix = lda(concatenated_evec_matrix, specpart_refined_partition)
            reduced_evec_matrix = Array(reduced_evec_matrix')
        end
        ...
        if isempty(partitions_vec)
            @warn "[specpart-features-kway] iteration $iter: tree_partition returned empty, skipping"
            continue
        end
        ...
        try run(`sh -c "rm -f *.part.$(num_parts)"`, wait=true) catch end
        for i in 1:length(partitions)
            (cutsize, balance) = golden_evaluator(hgraph, num_parts, partitions[i])
            push!(cutsizes, cutsize)
            if haskey(cut_dictionary, cutsize) == false
            ...
            @info "[specpart-features-kway] Refined partition $i with cutsize $cutsize $balance"
        end
        ...
        for i in 1:solns_to_pick
            push!(best_partitions, unique_partitions[sorted_partition_ids[i]])
            @info "[specpart-features-kway] partition picked with cutsize $(unique_cutsizes[sorted_partition_ids[i]])"
        end
        push!(best_partitions, partition)
        hgraph_contracted, clusters = overlay(best_partitions, hgraph)
        @info "Running optimal attempt partitioning**"
        refined_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
        cutsize = golden_evaluator(hgraph_contracted, num_parts, refined_partition)
        @info "specpart-features-kway cutsize recorded: $cutsize"
        partition_projected = zeros(Int, hgraph.num_vertices)
        for i in 1:length(clusters)
            cid = clusters[i]
            partition_projected[i] = refined_partition[cid]
        ...
        push!(global_cutsizes, cutsize[1])
            ...
            @warn "[specpart-features-kway] iteration $iter FAILED: $(sprint(showerror, iter_e))"
            try run(`sh -c "rm -f *.part.$(num_parts)"`, wait=true) catch end
            continue
        end
    end
    # global overlaying
    @info "[specpart-features-kway] running final round of overlay"
    push!(global_partitions, partition)
    hgraph_contracted, clusters = overlay(global_partitions, hgraph)
    @info "Running optimal attempt partitioning**"
    specpart_partition = optimal_partitioner(hmetis_path, ilp_path, hgraph_contracted, num_parts, ub_factor; seed=seed)
    cutsize = golden_evaluator(hgraph_contracted, num_parts, specpart_partition)
    @info "[specpart-features-kway] refined cutsize recorded: $cutsize"
    partition_projected = zeros(Int, hgraph.num_vertices)
    for i in 1:length(clusters)
        cid = clusters[i]
        partition_projected[i] = specpart_partition[cid]
    ...
    cutsize = golden_evaluator(hgraph, num_parts, partition_projected)
    if isempty(global_cutsizes)
        @info "specpart-features-kway cutsize recorded (no refine iters succeeded): $(cutsize[1])"
        try run(`sh -c "rm -f $(processed_hg_name)"`, wait=true) catch end
        return partition_projected, cutsize[1]
    end
    global_min_cut, idx = findmin(global_cutsizes)
    final_cut = 0
    ...
    post_refined_part = final_part
    if pre_refined_cut < post_refined_cut
        @info "specpart-features-kway cutsize recorded: $pre_refined_cut"
        return pre_refined_part, pre_refined_cut
    else
        @info "specpart-features-kway cutsize recorded: $post_refined_cut"
        return post_refined_part, post_refined_cut
    end
end

function specpart_run_with_features(hypergraph_file::String,
                    features::Matrix{Float64};
                    hypergraph_fixed_file::String = "",
                    hint_file::String = "",
                    imb::Int=2,
    ...
    line_log = repeat("=", 60)
    @info "$line_log"
    @info "SpecPart K-way with GIFT features**"
    @info "$line_log"
    @info "Hypergraph info**"
    @info "$line_log"
    hypergraph = read_hypergraph_file(hypergraph_file, hypergraph_fixed_file)
    @info "Num vertices $(hypergraph.num_vertices)"
    @info "Num hyperedges $(hypergraph.num_hyperedges)"
    @info "Features matrix size $(size(features))"
    @info "specpart_mode = $specpart_mode"
    (processed_hypergraph,
    original_indices,
    new_indices,
    unused_indices) = isolate_islands(hypergraph)
    ...
    # Map features to processed hypergraph vertices
    # original_indices[new_i] = orig_i, so processed vertex new_i corresponds to original vertex original_indices[new_i]
    n_proc = processed_hypergraph.num_vertices
    processed_features = zeros(Float64, n_proc, size(features, 2))
    for new_i in 1:n_proc
        orig_i = original_indices[new_i]
        if orig_i <= size(features, 1)
            processed_features[new_i, :] = features[orig_i, :]
        end
    end
    @info "Processed features size $(size(processed_features))"
    ...
    cutsize = 0
    if num_parts == 2
        refined_partition, cutsize = two_way_spectral_refine_with_features(hypergraph_file,
                                                    processed_partition,
                                                    processed_hypergraph,
                                                    metis_path,
                                                    imb,
    ...
    elseif num_parts > 2
        refined_partition, cutsize = k_way_spectral_refine_with_features(hypergraph_file,
                                                processed_partition,
                                                processed_hypergraph,
                                                metis_path,
                                                imb,
