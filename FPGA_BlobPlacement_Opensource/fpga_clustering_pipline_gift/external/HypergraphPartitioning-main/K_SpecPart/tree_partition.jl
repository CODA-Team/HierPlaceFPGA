...
function METIS_tree_partition(T::SimpleWeightedGraphs.SimpleGraph, 
    ...
    close(pfile)
    try; rm(gname; force=true); catch; end
    try; rm(pname; force=true); catch; end
    (cutsize, ~) = golden_evaluator(hgraph, num_parts, partition)
    ...
    for type in 1:types
        ...
        for i in 1:length(dims)
            ...
            for j in eachindex(evecs)
                ...
                @info "[tree_partition] type=$type evecs=$(evecs[j]) X_thr=$(size(X_thr)) ndims=$(ndims(X_thr))"
                ...
                # Check tree connectivity — disconnected trees cause BoundsError in euler tour / LCA.
                # akpw() can return trees where ne() >= nv-1 but the graph is still
                # disconnected from node 1's perspective (self-loops, duplicate edges, etc.).
                # Use connected_components as the definitive check.
                n_tree = SimpleWeightedGraphs.nv(tree)
                n_tree_edges = SimpleWeightedGraphs.ne(tree)
                if n_tree > 0
                    cc = SimpleWeightedGraphs.connected_components(tree)
                    if length(cc) > 1
                        @warn "[tree_partition] type=$type evecs=$(evecs[j]): tree has $(length(cc)) components (ne=$n_tree_edges, nv=$n_tree), skipping"
                        continue
                    end
                end
                # Sanity: tree must span all hypergraph vertices
                if n_tree != hgraph.num_vertices
                    @warn "[tree_partition] type=$type evecs=$(evecs[j]): tree nv=$n_tree != hgraph.num_vertices=$(hgraph.num_vertices), skipping"
                    continue
                end
                if n_tree_edges < n_tree - 1
                    @warn "[tree_partition] type=$type evecs=$(evecs[j]): tree has too few edges ($n_tree_edges < $(n_tree-1)), skipping"
                    continue
                end
                ...
                if (cutsize_tree < 1e09)
                    ...
                    tp_fail_count += 1
                    bt = catch_backtrace()
                    @warn "[tree_partition] type=$type evecs=$(evecs[j]) failed: $(sprint(showerror, tp_e))\n$(sprint(Base.show_backtrace, bt))"
                    continue
                end
            end
        end
    end
    @info "[tree_partition] summary: $tp_total_count attempts, $tp_fail_count failures, $(length(partitions)) partitions produced"
    return partitions
end