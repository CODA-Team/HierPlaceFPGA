...
function metis(fname::String, seed::Int, opts::String)
    ...
    script_path = joinpath(dirname(@__DIR__), "metis_script.sh")
    metis_r = `$script_path $fname $num_parts $ub_factor $seed $log_name`
    run(metis_r, wait=true)
    try; rm(log_name; force=true); catch; end
end
...
function METIS_Run(hypergraph::Hypergraph, tree::SimpleWeightedGraph, seed::Int, opts::String)
    ...
    i = 0
    pvec_full = zeros(Int, n_tree)

    for ln in eachline(f)
        i += 1
        if i <= n_tree
            pvec_full[i] = parse(Int, ln)
        end
    end

    close(f)

    # Tree may have more vertices than hypergraph (trailing isolated nodes).
    # Truncate to hypergraph.n for FindCutSize compatibility.
    pvec = pvec_full[1:hypergraph.n]

    cut_size = FindCutSize(pvec, hypergraph), pvec
    try; rm(fname; force=true); catch; end
    try; rm(fname * ".part.2"; force=true); catch; end
