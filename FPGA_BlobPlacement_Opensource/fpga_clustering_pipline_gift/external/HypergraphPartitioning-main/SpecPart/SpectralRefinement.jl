...
"""Safe file/dir removal — never throws on missing files."""
function safe_rm(paths::String...)
    for p in paths
        try
...
function hmetis(fname::String, Nparts::Int, UBfactor::Int, Nruns::Int, CType::Int, RType::Int, Vcycle::Int, Reconst::Int, dbglvl::Int, hmetis_exec::String = "./SpecPart/hmetis", pseed::Int = 0)
    # 1. 构造绝对路径，防止路径混淆
    if startswith(hmetis_exec, "./")
        real_exec = joinpath(pwd(), hmetis_exec[3:end])
    else
        real_exec = hmetis_exec
    end

    # 2. 检查可执行文件是否存在
    if !isfile(real_exec)
        @error "找不到 hmetis 程序: $real_exec"
        return
    end
    
    # 3. 使用 Julia 原生命令对象 (避免 sh -c 导致的参数错误)
    cmd = `$real_exec $fname $Nparts $UBfactor $Nruns $CType $RType $Vcycle $Reconst $dbglvl $pseed`
    
    #@info "调用本地 hmetis: $cmd"
    try
        run(cmd, wait=true)
    catch e
        @error "运行出错: $e"
...
#function hmetis(fname::String, Nparts::Int, UBfactor::Int, Nruns::Int, CType::Int, RType::Int, Vcycle::Int, Reconst::Int, dbglvl::Int, hmetis_exec::String = "/work/SpecPart/HypergraphPartitioning-main/SpecPart/hmetis")
#    hmetis_log = "hmetis_log.txt"
#    hmetis_err = "hmetis_err.txt"
#    cmd = Cmd([hmetis_exec, fname, string(Nparts), string(UBfactor), string(Nruns), string(CType), string(RType), string(Vcycle), string(Reconst), string(dbglvl)])
#    try
#        run(pipeline(cmd, stdout=hmetis_log, stderr=hmetis_err))
#        if isfile(hmetis_err)
#            err_content = read(hmetis_err, String)
#            if !isempty(err_content)
#                @warn "hMetis stderr: $err_content"
#            end
#            rm(hmetis_err; force=true)
#        end
#    catch e
#        if isfile(hmetis_err)
#            err_content = read(hmetis_err, String)
...
function GoldenEvaluator(hypergraph::Hypergraph, partition::Vector{Int}, max_capacity::Int, min_capacity::Int) #计算cutsize和判断是否在容量范围内以判断平衡
    cut_size = 0
    balance = false
...
function OverlayBasedClusteringAndILP(hg::String, ub_factor::Int, seeds::Vector{Int})
    ...
    for i in 1:length(seeds)
        ...
        println("Seed $(seeds[i]) → Cut = $(cut_list[i])")
    end
    #我添加的
    # 🟢 输出候选划分矩阵
    println("\n========== Partition Matrix (δ × n) ==========")
    println(partition_matrix)
    writedlm("debug_partition_matrix.txt", partition_matrix)

    #@info "CUTS :: $(cut_list)"
    t_clus = @elapsed  (hypergraph_clustered, incidence_struct_clustered, fixed_part_clustered, vwts_clustered, cclist, csize) = GenerateSpectralCommunities(partition_matrix, hypergraph_c, incidence_struct)

    # ================= [DEBUG] CLUSTER SIZE ANALYSIS (OverlayBasedClusteringAndILP) =================
    println("\n" * "="^70)
    println("[DEBUG] Cut Overlay Cluster Analysis (OverlayBasedClusteringAndILP)")
    println("="^70)
    
    total_clusters = length(csize)
    ones_count = count(x -> x == 1, csize)
    twos_count = count(x -> x == 2, csize)
    small_count = count(x -> x <= 3, csize)  # 大小<=3的cluster
    max_sz = maximum(csize)
    avg_sz = mean(csize)
    
    println("Total Clusters (Super Nodes): $total_clusters")
    println("Original Vertices:            $(hypergraph_c.HG.n)")
    println("Compression Ratio:            $(round(hypergraph_c.HG.n / total_clusters, digits=2))x")
    println("-"^70)
    println("Clusters with Size = 1 (Dust):  $ones_count  [$(round(ones_count/total_clusters*100, digits=2))%]")
    println("Clusters with Size = 2:         $twos_count  [$(round(twos_count/total_clusters*100, digits=2))%]")
    println("Clusters with Size <= 3:        $small_count [$(round(small_count/total_clusters*100, digits=2))%]")
    println("Largest Cluster Size:           $max_sz")
    println("Average Cluster Size:           $(round(avg_sz, digits=2))")
    println("-"^70)
    
    # 打印cluster大小分布
    sorted_sizes = sort(csize)
    preview_len = min(50, total_clusters)
    println("Smallest $preview_len sizes: $(sorted_sizes[1:preview_len])")
    
    # 如果有很多大小为1的cluster，发出警告
    if ones_count / total_clusters > 0.3
        println("\n⚠️  WARNING: High percentage of size-1 clusters detected!")
        println("   This indicates poor clustering / unbalanced overlay.")
    end
    println("="^70 * "\n")
    # ================= [DEBUG END] =================
     ...
     # 🟢 输出聚类后的超图结构
     println("\n========== 聚类后超图 (H_c) ==========")
     println("顶点数：", hypergraph_clustered.n)
     println("超边数：", hypergraph_clustered.e)
     println("前几个超边示例：")
     for i in 1:min(5, hypergraph_clustered.e)
         println("E[$i] = ", hypergraph_clustered.hedges[hypergraph_clustered.eptr[i]:(hypergraph_clustered.eptr[i+1]-1)])
     end
     ExportHypergraph(hypergraph_clustered, hg_name_clustered, fixed_part_clustered)
 
     # 🟢 同时保存聚类后超图的结构信息到文件
     open("debug_hypergraph_clustered.txt", "w") do io
         println(io, "Clustered Hypergraph Summary:")
         println(io, "Vertices: ", hypergraph_clustered.n)
         println(io, "Hyperedges: ", hypergraph_clustered.e)
         for i in 1:hypergraph_clustered.e
             vtxs = hypergraph_clustered.hedges[hypergraph_clustered.eptr[i]:(hypergraph_clustered.eptr[i+1]-1)]
             println(io, "E[$i]: ", join(vtxs, ", "))
         end
     end
     #我添加的
    ...
    if hypergraph_clustered.e <= 2000
        ...
        t_part = @elapsed hmetis(hg_name_clustered, 2, ub_factor, 10, 1, 1, 0, 1, 0, "./hmetis", 0) #"SpectralCommunityDetection/hmetis")
        pname = hg_name_clustered * ".part." * string(2)
        f = open(pname, "r")
    ...
    safe_rm(hg_name_clustered)
    cut_size = findCutsize(partition_vector, hypergraph_clustered, incidence_struct_clustered)

    return cut_size, t_clus+t_part
end

function SpectralHmetisRefinement(;refine_iters::Int = 4, solver_iters::Int = 20, hg::String = "", pfile::String = "", fg::String = "", Nparts::Int = 2, hyperedges_threshold::Int = 900, ub::Int = 5, nev::Int = 1, cycles::Int = 1, seed::Int = 0, best_solns::Int=10, pseed::Int=0, hmetis_exec::String=get(ENV, "HMETIS_EXEC", "hmetis"))
    ...
    t_hint = @elapsed begin 
        if isempty(pfile) == true
            ...
            let real_hmetis = startswith(hmetis_exec, "./") ? joinpath(pwd(), hmetis_exec[3:end]) : hmetis_exec
                run(`$real_hmetis $hypergraph_file $num_parts $ub_factor 50 1 1 0 1 0 $pseed`, wait=true)
            end
    ...
    # ================= [DEBUG START] PRINT CLUSTER SIZES =================
    println("\n" * "="^60)
    println("[DEBUG] SpecPart Overlay Analysis (via csize)")
    println("="^60)
    
    # csize 变量直接包含了 GenerateSpectralCommunities 返回的每个 Cluster 的大小
    total_clusters = length(csize)
    ones_count = count(x -> x == 1, csize)
    twos_count = count(x -> x == 2, csize)
    max_sz = maximum(csize)
    
    println("Total Super Nodes (Clusters): $total_clusters")
    println("Clusters with Size = 1 (Dust): $ones_count  [$(round(ones_count/total_clusters*100, digits=1))%]")
    println("Clusters with Size = 2: $twos_count")
    println("Largest Cluster Size: $max_sz")
    
    # 打印前 30 个最小的大小
    sorted_sizes = sort(csize)
    preview_len = min(30, total_clusters)
    println("Smallest $preview_len sizes: $(sorted_sizes[1:preview_len])")
    println("="^60 * "\n")
    # ================= [DEBUG END] =================
    @info "$line_log"
    ...
    #else
    @info "RUNNING HMETIS AS GOLDEN PARTITIONER"
    t_part = @elapsed hmetis(hg_name_clustered, num_parts, ub_factor, 50, 1, 1, 0, 1, 0, hmetis_exec, pseed)
    pname = hg_name_clustered * ".part." * string(Nparts)
    f = open(pname, "r")
