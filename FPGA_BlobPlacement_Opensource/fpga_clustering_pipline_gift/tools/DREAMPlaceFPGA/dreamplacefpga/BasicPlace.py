...
class PlaceDataCollectionFPGA(object):
    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            print(f"[MASK-DEBUG] lut_mask sum: {self.lut_mask.sum().item()}, flop_mask sum: {self.flop_mask.sum().item()}, flop_lut_mask sum: {self.flop_lut_mask.sum().item()}")
            ...
            # --- net weight anneal base (w0 from .wts) ---
            self.net_weights_init = self.net_weights.clone()
            self.net_weights_delta = self.net_weights_init - 1.0
...
class PlaceOpCollectionFPGA(object):
    ...
    def __init__(self):
        ...
        self.hpwl_op = None
        self.hpwl_weighted_op = None   # weighted HPWL (uses net weights)
...
#初始布局在这
        ...
        # ------------------------------------------------------------
        # (NEW) load initial placement from params.init_placement_file
        # The file is expected to have lines like:
        #   inst_name x y z [FIXED/...]
        # We only override nodes that exist in placedb.node_name2id_map.
        # ------------------------------------------------------------
        use_init_pl = False
        init_pl_path = getattr(params, "init_placement_file", "")
        init_pl_xy = {}   # nodeId -> (x, y)
        init_pl_z  = {}   # nodeId -> z (optional)

        if init_pl_path and os.path.isfile(init_pl_path):
            loaded = 0
            with open(init_pl_path, "r") as f:
                for line in f:
                    ...
                    if (not s) or s.startswith("#"):
                        continue
                    # allow both "name x y z" and "name x y z : N"
                    tokens = s.replace(":", " ").split()
                    if len(tokens) < 3:
                        continue
                    name = tokens[0]
                    if name not in placedb.node_name2id_map:
                        # your movable_init.pl 里可能有孤立行（比如只有数字），这里直接跳过即可
                        continue
                    node_id = placedb.node_name2id_map[name]
                    # DEBUG: print raw line & parsed xy for a few watch nodes
                    WATCH = {"inst_1000", "inst_10000"}
                    if name in WATCH:
                        logging.info("[DEBUG][InitPL] path=%s raw='%s' tokens=%s node_id=%d",
                                    init_pl_path, s, tokens, node_id)
                    # parse x,y,(z)
                    try:
                        x = float(tokens[1])
                        y = float(tokens[2])
            ...
            if loaded > 0:
                use_init_pl = True
                logging.info(f"[InitPlacement] Loaded {loaded} nodes from {init_pl_path}")
            else:
                logging.warning(f"[InitPlacement] init_placement_file provided but no valid nodes loaded: {init_pl_path}")

        # x position
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x

        if use_init_pl:
            # override from init_placement_file (only for nodes present in file)
            for node_id, (x, y) in init_pl_xy.items():
                self.init_pos[node_id] = x
            # also set z if provided (for later legalization)
            if hasattr(placedb, "node_z"):
                for node_id, z in init_pl_z.items():
                    placedb.node_z[node_id] = z

        elif params.global_place_flag and params.random_center_init_flag:  # original behavior
        ...
        # IMPORTANT:
        # If your init_placement_file coordinates are already LOWER-LEFT (typical .pl),
        # you should NOT shift by 0.5*size.
        # If your init_placement_file coordinates are CENTER, set params.init_placement_is_center=1.
        init_is_center = getattr(params, "init_placement_is_center", 0)
        if (not use_init_pl) or init_is_center:
        ...
        if use_init_pl:
            ...
            # DEBUG: verify final init_pos for watch nodes after assignment
            try:
                for nm in ["inst_1000", "inst_10000"]:
                    if nm in placedb.node_name2id_map:
                        nid = placedb.node_name2id_map[nm]
                        yy = float(self.init_pos[placedb.num_nodes + nid])
                        xx = float(self.init_pos[nid])
                        logging.info("[DEBUG][InitPOS] %s nid=%d init_pos=(%.6f, %.6f) init_is_center=%s size_y=%.6f",
                                    nm, nid, xx, yy,
                                    str(getattr(params, "init_placement_is_center", 0)),
                                    float(placedb.node_size_y[nid]))
            except Exception as e:
                logging.warning("[DEBUG][InitPOS] failed: %s", str(e))
        elif params.global_place_flag and params.random_center_init_flag:
                ...
                if i < 4:
                    ...
                    if filler_end-filler_beg > 0:
                        ...
                        for j, subregion in enumerate(region):
                            ...
                            nfill = int(sub_filler_end - sub_filler_beg)
                            if nfill <= 0:
                                continue

                            # Guard against invalid ranges (region smaller than filler size)
                            x_low = float(subregion[0])
                            x_high = float(subregion[2] - placedb.filler_size_x_fence_region[i])
                            y_low = float(subregion[1])
                            y_high = float(subregion[3] - placedb.filler_size_y_fence_region[i])
                            ...
                            x_start = int(placedb.num_physical_nodes + filler_beg + sub_filler_beg)
                            x_end = int(placedb.num_physical_nodes + filler_beg + sub_filler_end)
                            y_offset = self.init_pos.size // 2

                            # Clamp x-range to the first half of init_pos (x coordinates).
                            x_start = max(0, min(x_start, y_offset))
                            x_end = max(0, min(x_end, y_offset))
                            if x_end <= x_start:
                                continue

                            y_start = y_offset + x_start
                            y_end = y_offset + x_end

                            # Clamp y-range to the second half of init_pos (y coordinates).
                            y_start = max(y_offset, min(y_start, self.init_pos.size))
                            y_end = max(y_offset, min(y_end, self.init_pos.size))
                            if y_end <= y_start:
                                continue

                            x_n = x_end - x_start
                            y_n = y_end - y_start
                            nfill_eff = min(nfill, x_n, y_n)
                            if nfill_eff <= 0:
                                continue

                            self.init_pos[x_start:x_start + nfill_eff] = np.random.uniform(
                                low=x_low,
                                high=x_high,
                                size=nfill_eff
                            )

                            self.init_pos[y_start:y_start + nfill_eff] = np.random.uniform(
                                low=y_low,
                                high=y_high,
                                size=nfill_eff
                            )
        ...
        # ------------------------------------------------------------
        # FIX: re-apply init_placement_file AFTER filler initialization
        # This guarantees init_pos won't be overwritten before pos creation.
        # ------------------------------------------------------------
        if use_init_pl:
            for node_id, (x, y) in init_pl_xy.items():
                self.init_pos[node_id] = x
            ...
            # optional: quick sanity check for two watch nodes
            try:
                for nm in ["inst_1000", "inst_10000"]:
                    nid = placedb.node_name2id_map.get(nm, None)
                    if nid is None:
                        continue
                    xx = float(self.init_pos[nid])
                    yy = float(self.init_pos[placedb.num_nodes + nid])
                    logging.info("[DEBUG][InitPOS_FINAL_BEFORE_POS] %s nid=%d init_pos=(%.6f, %.6f)", nm, nid, xx, yy)
            except Exception as e:
                logging.warning("[DEBUG][InitPOS_FINAL_BEFORE_POS] failed: %s", str(e))
        ...
        # DEBUG: check pos right after creation
        try:
            with torch.no_grad():
                for nm in ["inst_1000", "inst_10000"]:
                    nid = placedb.node_name2id_map.get(nm, None)
                    if nid is None:
                        continue
                    xx = float(self.pos[0][nid].item())
                    yy = float(self.pos[0][placedb.num_nodes + nid].item())
                    logging.info("[DEBUG][POS_AFTER_CREATE] %s nid=%d pos=(%.6f, %.6f)", nm, nid, xx, yy)
        except Exception as e:
            logging.warning("[DEBUG][POS_AFTER_CREATE] failed: %s", str(e))
        ...
        #tt = time.time()
        # ------------------------------------------------------------
        # FIX: sync init_pos into self.pos (now self.pos exists)
        # ------------------------------------------------------------
        with torch.no_grad():
        ...
        # hpwl for reporting: always unweighted (all ones)
        unweighted_net_weights = torch.ones_like(self.data_collections.net_weights)
        self.op_collections.hpwl_op = self.build_hpwl(
            params, placedb, self.data_collections, self.op_collections.pin_pos_op, self.device,
            net_weights=unweighted_net_weights
        )

        # hpwl for algorithm/debug: weighted (matches net weights file)
        self.op_collections.hpwl_weighted_op = self.build_hpwl(
            params, placedb, self.data_collections, self.op_collections.pin_pos_op, self.device,
    ...
    def build_hpwl(self, params, placedb, data_collections, pin_pos_op, device, net_weights=None):
        """
