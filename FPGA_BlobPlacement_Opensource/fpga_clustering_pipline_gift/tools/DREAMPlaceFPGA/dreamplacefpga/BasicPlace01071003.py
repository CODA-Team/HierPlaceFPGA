    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            self.flat_net2pin_start_map = torch.from_numpy(placedb.flat_net2pin_start_map).to(device)
             # Debug net weights
            logging.warning(f"net_weights min: {np.min(placedb.net_weights)}, max: {np.max(placedb.net_weights)}")
            logging.warning(f"net_weights unique count: {np.unique(placedb.net_weights).size}")
