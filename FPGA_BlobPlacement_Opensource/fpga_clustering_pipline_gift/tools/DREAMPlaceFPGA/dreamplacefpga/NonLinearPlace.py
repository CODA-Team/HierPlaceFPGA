...
#
from TrajectoryTrace import CellTrajectoryTracer
import os 
import sys
import time 
import pickle
...
class NonLinearPlaceFPGA (BasicPlaceFPGA):
    ...
    def __call__(self, params, placedb):
        ...
        # ------------------------------------------------------------
        # DEBUG: dump positions before global placement
        # ------------------------------------------------------------
        try:
            with torch.no_grad():
                # only dump physical nodes (exclude fillers)
                num = int(placedb.num_physical_nodes)
                num_nodes = int(placedb.num_nodes)

                pos_t = self.pos[0]  # [x(0..num_nodes-1), y(num_nodes..2*num_nodes-1)]
                x = pos_t[:num].detach().cpu().numpy()
                y = pos_t[num_nodes:num_nodes + num].detach().cpu().numpy()

                # z usually stored in data_collections.node_z
                if hasattr(self.data_collections, "node_z") and self.data_collections.node_z is not None:
                    z = self.data_collections.node_z[:num].detach().cpu().numpy()
                else:
                    z = np.zeros(num, dtype=np.int32)

                names = placedb.node_names[:num]  # PlaceDB uses node_names to map instances :contentReference[oaicite:1]{index=1}

                # write to file under result_dir (by node id, avoid name/idx mismatch)
                out_dir = getattr(params, "result_dir", ".")
                os.makedirs(out_dir, exist_ok=True)
                out_pl = os.path.join(out_dir, "debug_pre_global_place.pl")

                with open(out_pl, "w") as f:
                    for name, nid in placedb.node_name2id_map.items():
                        if nid >= num:   # only physical nodes
                            continue
                        xx = float(pos_t[nid].item())
                        yy = float(pos_t[num_nodes + nid].item())
                        zz = int(self.data_collections.node_z[nid].item()) if hasattr(self.data_collections, "node_z") else 0
                        f.write(f"{name} {xx:.6f} {yy:.6f} {zz}\n")

                #logging.info("[DEBUG] dumped pre-GP placement to %s (num_physical_nodes=%d)", out_pl, num)

                # print first 20 *by id* (id=0..19), guaranteed consistent with init_pos/id
                k = min(20, num)
                logging.info("[DEBUG] first %d nodes before GP (by node_id):", k)
                for nid in range(k):
                    name = placedb.node_names[nid]
                    xx = float(pos_t[nid].item())
                    yy = float(pos_t[num_nodes + nid].item())
                    zz = int(self.data_collections.node_z[nid].item()) if hasattr(self.data_collections, "node_z") else 0
                    logging.info("  [id=%4d] %s  x=%.6f y=%.6f z=%d", nid, name, xx, yy, zz)


                # quick consistency check vs placedb initial arrays (if they exist)
                if hasattr(placedb, "node_x") and hasattr(placedb, "node_y"):
                    x0 = placedb.node_x[:num]
                    y0 = placedb.node_y[:num]
                    # node_x/y in placedb are numpy arrays (from read_pl / vivado map) :contentReference[oaicite:2]{index=2}
                    dx = np.max(np.abs(x - x0)) if num > 0 else 0.0
                    dy = np.max(np.abs(y - y0)) if num > 0 else 0.0
                    bad = int(np.sum((np.abs(x - x0) > 1e-3) | (np.abs(y - y0) > 1e-3)))
                    #logging.info(
                      #  "[DEBUG] pre-GP vs placedb init diff: max|dx|=%.6f max|dy|=%.6f bad_count(>1e-3)=%d/%d",
                     #   float(dx), float(dy), bad, num
                   # )
        except Exception as e:
            logging.warning("[DEBUG] failed to dump pre-GP placement: %s", str(e))

        # global placement 
        if params.global_place_flag: 
            # ===== trace first 10 cells from movable_init.pl =====
            tracer = None
            if int(getattr(params, "trace_cells_flag", 0)) == 1:
                trace_num = int(getattr(params, "trace_cells_num", 10))  # 默认10
                trace_interval = int(getattr(params, "trace_cells_interval", 1))

                # 默认用 params.init_placement_file；没有就用 movable_init.pl
                init_pl = getattr(params, "trace_cells_init_pl", "")
                if not init_pl:
                    init_pl = getattr(params, "init_placement_file", "")
                if not init_pl:
                    init_pl = "movable_init.pl"

                tracer = CellTrajectoryTracer(
                    placedb, params,
                    num_cells=trace_num,
                    interval=trace_interval,
                    init_pl_path=init_pl
                )
                tracer.record(0, self.pos[0])
            # ================================================
            ...
            for global_place_params in params.global_place_stages:
                ...
                # ------------------------------------------------------------
                # DEBUG: verify initial pos equals model.init_pos (movable only)
                # ------------------------------------------------------------
                #try:
                #    with torch.no_grad():
                #        pos0 = model.data_collections.pos[0].detach().cpu().numpy()
                #        init0 = model.init_pos  # numpy array saved inside PlaceObjFPGA

                #        n = int(placedb.num_movable_nodes)
                #        num_nodes = int(placedb.num_nodes)

#                        dx = np.abs(pos0[:n] - init0[:n])
#                        dy = np.abs(pos0[num_nodes:num_nodes + n] - init0[num_nodes:num_nodes + n])
#
#                       logging.info(
#                           "[DEBUG] pos0 vs model.init_pos (movable): max|dx|=%.6e max|dy|=%.6e bad_count(>1e-9)=%d/%d",
#                            float(dx.max() if n > 0 else 0.0),
#                            float(dy.max() if n > 0 else 0.0),
#                            int(((dx > 1e-9).sum() + (dy > 1e-9).sum())),
#                            int(2 * n),
#                        )
#                except Exception as e:
#                    logging.warning("[DEBUG] failed pos0 vs init_pos check: %s", str(e))

                
                #print("Model constructed in %g ms"%((time.time()-tt)*1000))
                # -----------------------------
                # Net-weight annealing: w0 (.wts) -> 1.0 during THIS global placement stage
                # -----------------------------
                stage_iter0 = iteration  # record the iteration when this GP stage starts

                # Prefer per-stage override from json; fallback to params; fallback to stage iteration
                anneal_iters = None
                if isinstance(global_place_params, dict):
                    anneal_iters = global_place_params.get("net_weight_anneal_iters", None)
                if anneal_iters is None and hasattr(params, "net_weight_anneal_iters"):
                    anneal_iters = params.net_weight_anneal_iters
                if anneal_iters is None and isinstance(global_place_params, dict):
                    anneal_iters = global_place_params.get("iteration", 0)
                anneal_iters = max(1, int(anneal_iters))

                # use cached delta = (w0 - 1)
                netw_delta = getattr(model.data_collections, "net_weights_delta", None)
                if netw_delta is None:
                    # backward compatibility if someone didn't add BasicPlace change yet
                    with torch.no_grad():
                        model.data_collections.net_weights_init = model.data_collections.net_weights.clone()
                        model.data_collections.net_weights_delta = model.data_collections.net_weights_init - 1.0
                    netw_delta = model.data_collections.net_weights_delta

                def anneal_net_weights(cur_iter: int):
                    t = max(0, cur_iter - stage_iter0)
                    prog = min(1.0, t / float(anneal_iters))  # 0 -> 1
                    s = 0.5 * (1.0 + math.cos(math.pi * prog))  # 1 -> 0 (cosine anneal)

                    with torch.no_grad():
                        nw = model.data_collections.net_weights
                        # nw = 1 + (w0 - 1) * s
                        nw.copy_(netw_delta).mul_(s).add_(1.0)
                        # ---- debug: print one net weight every 10 iters ----
                        debug_net_id = 0  # 随便选一个，例如第0条net
                        if (cur_iter % 10) == 0:
                            if nw.numel() > debug_net_id:
                                logging.info(
                                    "[NetWeightAnneal] iter=%d prog=%.4f s=%.4f net_id=%d w=%.6f",
                                    cur_iter, prog, s, debug_net_id, float(nw[debug_net_id].item())
                                )
                            else:
                                logging.info(
                                    "[NetWeightAnneal] iter=%d (no nets to print, numel=%d)",
                                    cur_iter, int(nw.numel())
                                )
                ...
                def metric_hpwl(m):
                    # prefer weighted HPWL for algorithmic decisions
                    return getattr(m, "hpwl_weighted", m.hpwl)
                    # stopping/divergence decisions should use REAL (unweighted) HPWL
                    #return m.hpwl
                    ...
                    with torch.no_grad():
                        if len(metrics) > 1: 
                            ...
                            if Lgamma_step > 100 and ((cur_metric.overflow[0] < placedb.targetOverflow[0] and cur_metric.overflow[1] < placedb.targetOverflow[1] and 
                                cur_metric.overflow[2] < placedb.targetOverflow[2] and cur_metric.overflow[3] < placedb.targetOverflow[3] and metric_hpwl(cur_metric) > metric_hpwl(prev_metric)) or 
                                ...
                                logging.info(
                                    ...
                                    % (Lgamma_step, cur_metric.overflow[0], placedb.targetOverflow[0],
                                       ...
                                       metric_hpwl(cur_metric), metric_hpwl(prev_metric),
                    ...
                    with torch.no_grad(): 
                        if len(metrics) > 1: 
                            ...
                            if (cur_metric.overflow[0] < placedb.targetOverflow[0] and cur_metric.overflow[1] < placedb.targetOverflow[1] and 
                                ...
                                metric_hpwl(cur_metric) > metric_hpwl(prev_metric) and (placedb.num_movable_nodes_fence_region[2:3].sum() == 0 or blockLegalIter >= 5)) or cur_metric.max_density[-1] < 1.0:
                                logging.info(
                                    ...
                                    (Llambda_density_weight_step,
                                     ...
                                     cur_metric.overflow[2], placedb.targetOverflow[2],
                                     cur_metric.overflow[3], placedb.targetOverflow[3], 
                                     metric_hpwl(cur_metric), metric_hpwl(prev_metric), cur_metric.max_density.max()))
                                return True
                    return False 
                ...
                def one_descent_step(Lgamma_step, Llambda_density_weight_step, Lsub_step, iteration, metrics, stop_mask=None):
                    ...
                    # update net weights before computing any objective/grad this step
                    anneal_net_weights(iteration)

                    # move any out-of-bound cell back to placement region 
                    self.op_collections.move_boundary_op(pos)
                    # ---- print force norms (wirelength / density) ----
                    wl_f, den_f = model.wl_density_force_norms(pos)
                    logging.info(
                         "[Iter %d] force_norm1: wl=%.3E density=%.3E",
                         iteration, float(wl_f.item()), float(den_f.item())
                    )
                    optimizer.zero_grad()
                    cur_metric.evaluate(placedb, eval_ops, pos, model.data_collections)
                    with torch.no_grad():
                        if hasattr(self.op_collections, "hpwl_weighted_op") and self.op_collections.hpwl_weighted_op is not None:
                            cur_metric.hpwl_weighted = self.op_collections.hpwl_weighted_op(pos)
                    model.overflow = cur_metric.overflow.data.clone()
                    with torch.no_grad():
                        gamma_val = float(model.gamma.item())
                        dw = model.density_weight.detach().cpu().numpy()
                        logging.info("[Iter %d] gamma=%.6e density_weight=%s",
                                    iteration, gamma_val,
                                    np.array2string(dw, precision=3, separator=', '))
                    ...
                    if torch.isnan(pos).any() or torch.isinf(pos).any():
                        logging.error("NaN/Inf detected at iter %d" % iteration)
                        raise RuntimeError("NaN/Inf in pos")
                ...
                def check_divergence(x, window=50, threshold=0.05):
                    ...
                    wl_mean = np.mean(x[:,0])
                    best_wl = metric_hpwl(best_metric[0]).item()
                    wl_ratio, overflow_ratio = (wl_mean - best_wl) / best_wl, (overflow_mean - max(placedb.targetOverflow.max(), best_metric[0].overflow.max().item()))/best_metric[0].overflow.max().item()
                ...
                for Lgamma_step in range(model.Lgamma_iteration):
                    ...
                    for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                        ...
                        if params.routability_opt_flag and num_area_adjust < params.max_num_area_adjust and Llambda_metrics[-1][-1].overflow[0:2].max() < params.node_area_adjust_overflow: 
                            ...
                            logging.info(f"[DEBUG-BEFORE-ADJUST] adjust_resource_area_flag={adjust_resource_area_flag}, adjust_route_area_flag={adjust_route_area_flag}, adjust_pin_area_flag={adjust_pin_area_flag}")
                            ...
                            if adjust_area_flag: 
                                ...
                                #logging.info("density_weight = [%s]" % ", ".join(["%.3E" % i for i in model.density_weight.cpu().numpy().tolist()]))
                                logging.warning(
                                     "[RESTART] iter=%d (Lgamma=%d Llambda=%d Lsub=%d) reason=restart_Llambda_reset_ops_and_density_weight",
                        ...
                        if placedb.num_movable_nodes_fence_region[2:4].max() > 0 and Llambda_metrics[-1][-1].overflow[0] < placedb.targetOverflow[0] and Llambda_metrics[-1][-1].overflow[1] < placedb.targetOverflow[1] and Llambda_metrics[-1][-1].overflow[2] < placedb.targetOverflow[2] and Llambda_metrics[-1][-1].overflow[3] < placedb.targetOverflow[3]:
                            ...
                            #logging.info("density_weight = [%s]" % ", ".join(["%.3E" % i for i in model.density_weight.cpu().numpy().tolist()]))
                            logging.warning(
                                     "[RESTART] iter=%d (Lgamma=%d Llambda=%d Lsub=%d) reason=restart_after_DSPRAM_legalization_and_lock",
                                     iteration, Lgamma_step, Llambda_density_weight_step, Lsub_step
                                )
                            # load state to restart the optimizer 
                            optimizer.load_state_dict(initial_state)
...
########################################
                ...
                if last_metric.overflow.max() > max(self.targetOverflow.max(), best_metric[0].overflow.max()) and metric_hpwl(last_metric) > metric_hpwl(best_metric[0]):
                    self.pos[0].data.copy_(best_pos[0].data)
                    logging.error(
                        "possible DIVERGENCE detected, roll back to the best position recorded"
                    )
            ...
            # ---- dump trace results after all global_place_stages finished ----
            if tracer is not None:
                path = "%s/%s" % (params.result_dir, params.design_name())
                out_dir = "%s/plot" % path
                tracer.finalize(
                    out_dir,
                    prefix=getattr(params, "trace_cells_prefix", "trace_cells"),
                    bounds=(placedb.xl, placedb.xh, placedb.yl, placedb.yh)
                )
            # ==========================================================
