...
class NonLinearPlaceFPGA (BasicPlaceFPGA):
    ...
    def __call__(self, params, placedb):
        ...
        if params.global_place_flag: 
            ...
            for stage_id, global_place_params in enumerate(params.global_place_stages):
                ...
                # [MOD] stage 0: disable update_mask/lock_mask (let all regions update)
                if stage_id == 0:
                    with torch.no_grad():
                        if len(placedb.region_boxes) > 0:
                            dev = self.data_collections.pos[0].device
                            model.update_mask = torch.ones(len(placedb.region_boxes), dtype=torch.bool, device=dev)
                            model.lock_mask   = torch.zeros(len(placedb.region_boxes), dtype=torch.bool, device=dev)
                # ------------------------------------------------------------
