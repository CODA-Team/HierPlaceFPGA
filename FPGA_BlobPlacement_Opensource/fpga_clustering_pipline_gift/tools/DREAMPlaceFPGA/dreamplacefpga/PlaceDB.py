...
class PlaceDBFPGA (object):
    ...
    def load_net_weights_from_file(self, weight_file):
        """
        weight_file format: <net_name> <float_weight>
        lines starting with # are comments; blank lines ignored
        """
        if weight_file is None or str(weight_file).strip() == "":
            return

        if not os.path.isfile(weight_file):
            raise FileNotFoundError(f"net weight file not found: {weight_file}")

        # 默认已在外面初始化成全 1，这里只覆盖出现的 net
        unknown = 0
        loaded = 0
        ...
        with open(weight_file, "r", encoding="utf-8") as f:
            for lineno, line in enumerate(f, start=1):
                ...
                parts = line.split()
                if len(parts) < 2:
                    raise ValueError(f"{weight_file}:{lineno} invalid line (need: net_name weight): {line}")

                net_name = parts[0]
                try:
                    w = float(parts[1])
                except Exception:
                    raise ValueError(f"{weight_file}:{lineno} invalid weight: {parts[1]}")

                # net_name -> net_id
                if net_name in self.net_name2id_map:
                    net_id = self.net_name2id_map[net_name]
                    self.net_weights[net_id] = w
                    loaded += 1
                else:
                    unknown += 1

        logging.info(f"Loaded {loaded} net weights from {weight_file} (unknown nets: {unknown})")
    ...
    def initialize_from_rawdb(self, params):
        ...
        # 新增：如果 params 里提供了 net_weight_file，就加载覆盖
        if hasattr(params, "net_weight_file") and params.net_weight_file:
            self.load_net_weights_from_file(params.net_weight_file)
    ...
    def get_io_sites(self, IOB_col, BUFGCE_col):
        ...
        for x in x_indices:
            ...
            site_x += 1
            site_y = 0
        # x_indices 是 IOB columns 的索引列表
        # 如果 cluster layout 太小，可能只有 0/1 列 IOB，会导致越界
        if len(x_indices) < 2:
            # fallback：复制第一个，保证后续代码不崩
            if len(x_indices) == 1:
                x_indices = [x_indices[0], x_indices[0]]
                print("[WARNING] Only one IOB column found in scl. Duplicate it for IO site mapping.")
            else:
                # 连一列都没有：直接返回空 mapping 或者跳过
                print("[WARNING] No IOB columns found in scl. Skip IO site mapping.")
                return io_sitemap  # 或者 return None（看调用方怎么处理）
        # Odd points 
