...
class PlaceObjFPGA(nn.Module):
    ...
    def wl_density_force_norms(self, pos):
        """
        返回 wirelength / density 两项对 pos 的“力”(梯度) 的范数（这里用 L1 norm）
        注意：density 项要和 obj_fn 的写法一致：density*(1+quad*...); 再做 density_weight_u.dot(...)
        """

        # --- 1) wirelength term ---
        wl = self.op_collections.wirelength_op(pos)

        # --- 2) density term (match obj_fn) ---
        density_raw = self.op_collections.fence_region_density_merged_op(pos)

        # 保证 quad_penalty_coeff / init_density 初始化逻辑与 obj_fn 一致
        if self.init_density is None:
        ...
        density = density_raw * (1 + self.quad_penalty_coeff * density_raw)
        density_term = self.density_weight_u.dot(density)

        # --- grads (forces) ---
        # 先算 wl 的梯度，保留图再算 density_term
        g_wl = torch.autograd.grad(wl, pos, retain_graph=True, create_graph=False)[0]
        g_den = torch.autograd.grad(density_term, pos, retain_graph=False, create_graph=False)[0]

        wl_force = g_wl.norm(p=1)
        den_force = g_den.norm(p=1)
        return wl_force, den_force
    ...
    def build_update_density_weight(self, params, placedb, algo="overflow"):
        ...
        def update_density_weight_op_hpwl(cur_metric, prev_metric, iteration):
            ...
            with torch.no_grad():
                def _metric_hpwl(m):
                    ...
                    # Use REAL (unweighted) HPWL for updating density weight (lambda).
                    # This avoids coupling lambda updates to dynamic net reweighting / annealing.
                    return m.hpwl

                delta_hpwl = _metric_hpwl(cur_metric) - _metric_hpwl(prev_metric)
                if delta_hpwl < 0:
        ...
        def update_density_weight_op_overflow(cur_metric, prev_metric, iteration):
            ...
            with torch.no_grad():
                ...
                # >>> DBG: cache density_norm L2 and step size (before)
                density_norm_l2 = float(density_norm.norm(p=2).item())
                dw_step_before = float(self.density_weight_step_size.item() if torch.is_tensor(self.density_weight_step_size) else self.density_weight_step_size)
                # <<< DBG
                ...
                # >>> DBG: cache step size (after)
                dw_step_after = float(self.density_weight_step_size.item() if torch.is_tensor(self.density_weight_step_size) else self.density_weight_step_size)
                # <<< DBG
                #content += ", Rate = %g" % (rate)
                ...
                # >>> DBG: compute "current" mask every time (do NOT change algorithm state)
                cur_update_mask = (cur_metric.overflow >= self.targetOverflow)
                # initialize stored masks only once (original behavior)
                if(self.update_mask is None):
                    self.update_mask = cur_update_mask
                    self.lock_mask = cur_metric.overflow < self.targetOverflow

                # >>> DBG PRINT
                try:
                    gp_iter = int(getattr(cur_metric, "iteration", -1))
                except Exception:
                    gp_iter = -1

                logging.info(
                    "[DW-DBG] gp_iter=%d Llambda_iter=%d "
                    "overflow=%s targetOverflow=%s "
                    "update_mask(cur)=%s update_mask(stored)=%s "
                    "density_norm_L2=%.3e dw_step=%.3e->%.3e",
                    gp_iter, int(iteration),
                    cur_metric.overflow.detach().cpu().numpy().tolist(),
                    self.targetOverflow.detach().cpu().numpy().tolist(),
                    cur_update_mask.detach().cpu().to(torch.int32).numpy().tolist(),
                    (self.update_mask.detach().cpu().to(torch.int32).numpy().tolist() if self.update_mask is not None else None),
                    density_norm_l2,
                    dw_step_before, dw_step_after
                )
                # <<< DBG PRINT
    ...
    def base_gamma(self, params, placedb):
        ...
        # ==========================================
        # Gentle gamma update: reduce K coefficient
        # ==========================================
        GAMMA_GENTLE_FACTOR = 1  # 降低gamma对overflow的敏感度 (推荐0.3-0.5)
        ...
        for i in range(len(placedb.region_boxes)):
            ...
            #   b = 1.0 - k
            original_K = 2.0/(1.0 - placedb.targetOverflow[i])
            gentle_K = original_K * GAMMA_GENTLE_FACTOR  # 应用平缓因子
            self.WLGammaK.append(gentle_K)
            self.WLGammaB.append(1.0 - gentle_K)
            
            logging.info(
                "[GAMMA_GENTLE] Region %d: K=%.3f (original %.3f, factor=%.2f), B=%.3f",
                i, gentle_K, original_K, GAMMA_GENTLE_FACTOR, 1.0 - gentle_K
            )
    ...
    def build_clustering_compatibility_ff_map(self, params, placedb, data_collections):
        ...
        # Handle ctrlSets dimension check
        if placedb.ctrlSets.ndim == 2 and placedb.ctrlSets.shape[0] > 0:
        ...
        return clustering_compatibility.FFCompatibility(
            ...
            num_bins_x=bins_x,
            num_bins_y=bins_y,
            num_bins_ck=num_bins_ck,
            num_bins_ce=num_bins_ce,
            xl=placedb.xl,
            yl=placedb.yl,
            xh=placedb.xh,
            yh=placedb.yh,
    ...
    def build_multi_fence_region_density_op(self):
        ...
        def merged_density_overflow_op(pos):
            ...
            overflow_list = torch.stack(overflow_list)
            max_density_list = torch.stack(max_density_list)

            # ===== DBG: print once =====
            if not hasattr(self, "_dbg_print_density_once"):
                self._dbg_print_density_once = True
            if self._dbg_print_density_once:
                logging.info("[DBG] overflow_list = %s", overflow_list.detach().cpu().numpy())
                logging.info("[DBG] max_density_list = %s", max_density_list.detach().cpu().numpy())
                # pos 是 [x_all, y_all] 拼起来的
                num_nodes = self.placedb.num_nodes
                num_movable = self.placedb.num_movable_nodes

                x = pos[:num_nodes]
                y = pos[num_nodes:num_nodes+num_nodes]

                xm = x[:num_movable].detach().cpu()
                ym = y[:num_movable].detach().cpu()
                logging.info("[DBG] movable x range: [%g, %g], y range: [%g, %g]",
                    float(xm.min()), float(xm.max()), float(ym.min()), float(ym.max()))
                self._dbg_print_density_once = False
                # ===========================
