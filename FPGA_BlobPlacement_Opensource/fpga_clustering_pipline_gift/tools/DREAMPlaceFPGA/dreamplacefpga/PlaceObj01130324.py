...
class PlaceObjFPGA(nn.Module):
    ...
    def initialize_density_weight(self, params, placedb):
        ...
        #### record initial density
        #### [MOD] use REAL overflow instead of density penalty (or "assume 1")
        with torch.no_grad():
            if hasattr(self.op_collections, "fence_region_density_overflow_merged_op") and \
               self.op_collections.fence_region_density_overflow_merged_op is not None:
                overflow_list, _ = self.op_collections.fence_region_density_overflow_merged_op(self.data_collections.pos[0])
                self.init_density = overflow_list
            else:
                # fallback: keep old behavior
                self.init_density = torch.stack(density_list)

        # [MOD] avoid div-by-zero / too-small overflow causing huge precond
        eps = 1e-6
        self.init_density = torch.clamp(self.init_density, min=eps)
