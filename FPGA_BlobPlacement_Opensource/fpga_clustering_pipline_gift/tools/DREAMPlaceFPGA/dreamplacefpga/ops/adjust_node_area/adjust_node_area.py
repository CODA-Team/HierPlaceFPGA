    ...
    def forward(self, pos, node_size_x, node_size_y, pin_offset_x,
        ...
        with torch.no_grad():
            ...
            if adjust_route_area_flag:
                ...
                logger.info(f"[DEBUG-ROUTE-CALC] route_opt_area computed, mean: {route_opt_area.mean().item()}, max: {route_opt_area.max().item()}, min: {route_opt_area.min().item()}")
            ...
            if adjust_pin_area_flag:
                ...
                logger.info(f"[DEBUG-PIN-CALC] pin_opt_area computed, mean: {pin_opt_area.mean().item()}, max: {pin_opt_area.max().item()}, min: {pin_opt_area.min().item()}")
            ...
            if adjust_resource_area_flag:
                area_diff = (resource_areas[:self.num_movable_nodes] - old_movable_area) * self.flop_lut_mask[:self.num_movable_nodes]
                logger.info(f"[DEBUG] area_diff sum (before relu): {area_diff.sum().item()}, positive count: {(area_diff > 0).sum().item()}")
                resource_area_increment_ratio = F.relu(area_diff).sum() / old_movable_area_sum
            ...
            if adjust_route_area_flag:
                route_diff = (route_opt_area - old_movable_area) * self.flop_lut_mask[:self.num_movable_nodes]
                logger.info(f"[DEBUG-ROUTE] route_diff sum (before relu): {route_diff.sum().item()}, positive count: {(route_diff > 0).sum().item()}, route_opt_area mean: {route_opt_area.mean().item()}, old_area mean: {old_movable_area.mean().item()}")
                route_area_increment_ratio = F.relu(route_diff).sum() / old_movable_area_sum
