...
void PlaceDB::add_bookshelf_node(std::string& name, std::string& type) 
{
    ...
    else if (limbo::iequals(type, "dffeas"))
    {
      node_name2id_map.insert(std::make_pair(name, mov_node_names.size()));
      flop_indices.emplace_back(mov_node_names.size());
      mov_node_names.emplace_back(name);
    ...
    else if (type_lower.rfind("lcell_comb", 0) == 0)
    {
      int inferred_lut(6);
      std::string suffix = type_lower.substr(std::string("lcell_comb").size());
      if (!suffix.empty()
              && std::all_of(suffix.begin(), suffix.end(),
                  [](unsigned char c) { return std::isdigit(c) != 0; }))
      {
        inferred_lut = std::stoi(suffix);
      }
      inferred_lut = std::max(0, std::min(6, inferred_lut));
      int inferred_cluster_lut = std::max(0, inferred_lut - 1);

      node_name2id_map.insert(std::make_pair(name, mov_node_names.size()));
      mov_node_names.emplace_back(name);
      mov_node_types.emplace_back(type);
      node2fence_region_map.emplace_back(0);
      mov_node_size_x.push_back((inferred_lut >= 4) ? sqrt0p125 : sqrt0p0625);
      mov_node_size_y.push_back((inferred_lut >= 4) ? sqrt0p125 : sqrt0p0625);
      mov_node_x.emplace_back(0.0);
      mov_node_y.emplace_back(0.0);
      mov_node_z.emplace_back(0);
      lut_type.emplace_back(inferred_lut);
      cluster_lut_type.emplace_back(inferred_cluster_lut);
      m_numLUT += 1;
      ++num_movable_nodes;
    }
    ...
    else if (type_lower.find("m9k") != std::string::npos
            || type_lower.find("m20k") != std::string::npos
            || type_lower.find("m144k") != std::string::npos)
    {
      node_name2id_map.insert(std::make_pair(name, mov_node_names.size()));
      mov_node_names.emplace_back(name);
      mov_node_types.emplace_back(type);
    ...
    else if (limbo::iequals(type, "io") || limbo::iequals(type, "pll"))
    {
