...
from solution_dataloader import SolutionGraphDataset, collate_solution_graphs
...
def init_model(args):
    """
    Compute input dims from hyperparameters (no graph parsing in init).
    """
...
def _weighted_precision_recall_f1(preds, labels, weights):
    """Compute quality-weighted precision, recall, F1. weights: same shape as preds/labels."""
    pred_pos = (preds == 1)
    true_pos = (labels == 1)
...
def evaluate_grouped(model, dataset, indices, device, loss_fn, args, threshold=0.5, return_predictions=False):
    """
    Evaluation with design-grouped inference: run GNN once per design (ub_isolate=True)
    or once per (design, ub) group (ub_isolate=False), then predict per solution.
    Returns (avg_loss, precision, recall, f1, w_prec, w_rec, w_f1) where w_* are None unless
    --eval_weighted_metrics. Optionally preds_by_sol for save_predictions.
    """
    model.eval()
    ...
    # Group solution indices
    if ub_isolate:
        groups = {}  # design_idx -> [sol_idx, ...]
        for idx in indices:
            di = dataset.solution_infos[idx]["design_idx"]
            groups.setdefault(di, []).append(idx)
    else:
        groups = {}  # (design_idx, ub_val) -> [sol_idx, ...]
        for idx in indices:
    ...
    with torch.no_grad():
        for group_key, sol_indices in groups.items():
            design_idx = group_key[0] if isinstance(group_key, tuple) else group_key

            # Build graph for this group (structural + ub in net feat when not ub_isolate)
            ...
            if ub_isolate:
                # One prediction per solution (different ub each time)
                ...
                # One prediction for the whole group (same ub); different labels per solution
...
def train(args,model):
    # -------------------------------------------------------------
    # 1. Load dataset: one structural graph per design, cached per-solution tensors
    # -------------------------------------------------------------
    dataset = GraphDataset(
        ...
        ub_isolate=getattr(args, 'ub_isolate', False),
    ...
    # -------------------------------------------------------------
    # 2. Split by solution (80% train, 10% val, 10% test)
    # -------------------------------------------------------------
    total_size = len(dataset)
    if total_size < 3:
        print(f"Dataset too small for standard split (total_size={total_size}), "
              f"using the same graphs for train/val/test.")
        train_set = dataset
        val_set = dataset
        test_set = dataset
    else:
    ...
    # -------------------------------------------------------------
    # 3. Extract solution indices for each split; create DataLoaders
    # -------------------------------------------------------------
    def _subset_to_indices(subset):
        """Convert random_split Subset to list of solution indices."""
    ...
    # DataLoader yields (batched_g, sol_indices). Each batch may mix solutions from
    # different designs; build_solution_graph clones the design structure per solution.
    train_loader = DataLoader(
        SolutionGraphDataset(dataset, train_indices),
        batch_size=args.batch_size,
        shuffle=True,
        collate_fn=collate_solution_graphs,
        num_workers=8,
        pin_memory=True,
        persistent_workers=True,
    )
    # Val/test use design-grouped evaluate (no DataLoader); train keeps DataLoader
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_g, sol_indices in train_loader:
            ...
            # Weighted BCE: higher weight for better-quality solutions (lower cutsize)
            # quality_weight is (num_nets, 1), replicated per net in the batch
            weights = batch_g.nodes["net"].data["quality_weight"]
            #print(weights.detach().cpu().numpy().tolist())
            raw_loss = loss_fn_none(logits, labels)  # (N, 1)
            loss = (raw_loss * weights).sum() / (weights.sum() + 1e-8)
        ...
        # --- Validation (design-grouped: GNN once per design / per (design,ub)) ---
        if epoch % 5 == 0:
    ...
    # -------------------------------------------------------------
    # 5. Final test (load best checkpoint)
    # -------------------------------------------------------------
    print("\n--- Final Testing ---")
    ...
    # -------------------------------------------------------------
    # 6. Optional: write per-solution prediction files (net_id cut_or_not per line)
    # -------------------------------------------------------------
