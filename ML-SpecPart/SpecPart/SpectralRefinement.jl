...
using JuMP
#using Cbc
#using Gurobi  # 注释掉商业求解器，使用hmetis代替
using LinearAlgebra
...
function OverlayBasedClusteringAndILP(hg::String, ub_factor::Int, seeds::Vector{Int})
    ...
    # 修改：总是使用hmetis，不使用ILP求解器
    # 原来的逻辑：小超图(<2000条边)用ILP，大超图用hmetis
    # 现在：所有情况都使用hmetis
    
    @info "使用 HMETIS 作为划分器"
    t_part = @elapsed hmetis(hg_name_clustered, 2, ub_factor, 10, 1, 1, 0, 1, 0, "./hmetis")
